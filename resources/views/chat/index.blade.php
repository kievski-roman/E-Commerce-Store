...
<x-app-layout>
    ...
    <div class="p-6">
        <ul id="messages" class="space-y-2 mb-4"></ul>

        <form id="chatForm">
            @csrf
            <input id="text" name="message" class="border p-2 rounded w-full" placeholder="Напиши сообщение..." />
    ...
    <script type="module">
        ...
        function append(author, text, timeIso) {
            const li = document.createElement('li');
            const time = new Date(timeIso).toLocaleTimeString();
            li.textContent = `${author}: ${text} (${time})`;
            messagesBox.appendChild(li);
            messagesBox.scrollTop = messagesBox.scrollHeight;
        }

        (async () => {
            const res = await fetch(`{{ route('chat.feed') }}`, { headers: { 'Accept': 'application/json' } });
            if (!res.ok) return console.error(await res.text());
            const data = await res.json();
            data.forEach(m => append(m.user.name, m.text, m.sent_at));
        })();

        window.Echo.channel('chat')
            .listen('.MessageSent', (e) => {
                append(e.user.name, e.message, e.sent_at);
            });
